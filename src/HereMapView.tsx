...
import { generateMockData, MARKER_COUNT_OPTIONS, type MockVehicle, type MockVisit } from './data'
...
function fitBoundsToMarkers(map: H.Map, currentVisits: MockVisit[]) {
  ...
  let rect = new H.geo.Rect(first.latitude, first.longitude, first.latitude, first.longitude)
...
export function HereMapView() {
  ...
  const { vehicles, visits } = useMemo(() => generateMockData(markerCount), [markerCount])
  ...
  const handleMarkerCountChange = (count: number) => {
    const map = hereMapRef.current

    if (count === markerCount) return
    ...
    if (map) {
      ...
      if (originsGroupRef.current) {
        ...
        originsGroupRef.current = null
      }
      hereMarkersRef.current = []
  ...
  useLayoutEffect(() => {
    ...
    const engineType = (H.Map.EngineType as unknown as Record<string, number>)['HARP']

    const style = new (H.map.render as any).harp.Style(HERE_MAP_STYLES)
    const vectorTileService = (platform as any).getOMVService()
    const vectorTileProvider = new (H.service as any).omv.Provider(vectorTileService, style, { engineType })
    const vectorTileLayer = new H.map.layer.TileLayer(vectorTileProvider, { max: 22, min: 0 } as never)
    ...
    const defaultLayers = platform.createDefaultLayers({ engineType, pois: true } as never)
  ...
  // Create/recreate markers when visits or vehicles change
  useEffect(() => {
    const map = hereMapRef.current
    if (!map) return

    // Clear old markers
    console.log('clear old markers')
    if (markersGroupRef.current) {
      map.removeObject(markersGroupRef.current)
      markersGroupRef.current = null
    }
    ...
    const newMarkers: HereMarkerData[] = visits.map((visit, index) => {
      ...
      const marker = new H.map.Marker(new H.geo.Point(visit.latitude, visit.longitude), {
        icon,
        volatility: true,
        data: { id: `marker-${index}`, vehicleId: visit.vehicleId, visitId: visit.id },
      } as never)
    ...
    vehicles.forEach((vehicle) => {
      ...
      const marker = new H.map.Marker(new H.geo.Point(vehicle.originLat, vehicle.originLng), {
        icon,
        volatility: true,
  ...
  useEffect(() => {
    ...
    hereMarkersRef.current.forEach((md) => {
      ...
      const isDimmed = !showAllRoutes && selectedVehicleId != null && md.vehicleId !== selectedVehicleId && !isSelected
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
              <div
                style={{
                  ...
                  borderTopColor: '#2A16DA',
                  borderRadius: '50%',
                  animation: 'spin 0.8s linear infinite',
                }}
              />
              <span style={{ fontSize: 14, color: '#333', fontWeight: 500 }}>Cargando markers...</span>
            </div>
          </div>
        )}
      </div>
...
function Sidebar({
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: 0, fontSize: 13, color: '#888' }}>{markerCount} markers / 10 vehículos</p>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        <label style={{ fontSize: 13, color: '#aaa' }}>Cantidad de markers</label>
        <select
          value={markerCount}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <label style={{ fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
          <input type="checkbox" checked={showAllRoutes} onChange={onToggleShowAll} />
      ...
      {selectedVisitCount > 0 && (
        ...
        >
          <span style={{ fontSize: 13 }}>{selectedVisitCount} visitas seleccionadas</span>
          <button
        ...
        {vehicles.map((vehicle) => (
          <button
            ...
            style={{
              ...
              gap: 10,
              padding: '8px 12px',
              border: selectedVehicleId === vehicle.id ? `2px solid ${vehicle.color}` : '2px solid transparent',
              borderRadius: 8,
              background: selectedVehicleId === vehicle.id ? '#16213e' : 'transparent',
              color: '#e0e0e0',
              cursor: 'pointer',
              textAlign: 'left',
          ...
          >
            ...
            <span style={{ fontSize: 12, color: '#888' }}>{vehicleVisitCounts[vehicle.id] || 0}</span>
          </button>
        ))}
      </div>
    </div>
